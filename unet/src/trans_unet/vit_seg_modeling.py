...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        # 在patch_embedding之前再加一个卷积层再次减半高宽来适配DRIVE数据集的大小
        ...
        self.dropout = Dropout(config.transformer["dropout_rate"])
        # 线性层，用于调整additional_conv并且patch_embeddings之后的token的维度
        self.linear_projection = nn.Linear(225, n_patches)
...
class DecoderBlock(nn.Module):
    ...
    def forward(self, x, skip=None):
        ...
        if skip is not None:
            ...
            print(f'before skip.shape: {skip.shape}')
            ...
            print(f'after skip.shape: {skip.shape}')
        x = self.conv1(x)
        x = self.conv2(x)
        return x
...
class DecoderCup(nn.Module):
    ...
    def forward(self, hidden_states, features=None):
        ...
        print(f"3333333x.shape  {x.shape}")
        # 上采样到 [1, 512, 56, 56]的一半，等会再上采样
        x = F.interpolate(x, size=(28, 28), mode='bilinear', align_corners=True)
        print(f"44444444x.shape  {x.shape}")
...
class VisionTransformer(nn.Module):
    def __init__(self, config, img_size=224, num_classes=21843, zero_head=False, vis=False):
        ...
        # 为了方便drive数据集加的，使得(565, 584)->(224, 224)
    ...
    def forward(self, x):
        ...
        # # 在这里将特征图的尺寸从 (566, 584) 调整到 (565, 584)
        # x = F.interpolate(x, size=(565, 584), mode='bilinear', align_corners=True)
